...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1, mlType=None, model=None):
    ...
    if every > 25 and len(frames_list) < 1000:  # this is faster for every > 25 frames and can fit in memory
        ...
        frame_ts=vr.get_frame_timestamp(frames_list)
        ...
        for index in range(start, end):  # lets loop through the frames until the end
            ...
            if index % every == 0:  # if this is a frame we want to write out based on the 'every' argument
                frame_ts=vr.get_frame_timestamp([index])
                save_path = os.path.join(frames_dir, video_filename,
                                         "{:010d}".format(index)+"_ts_"+str(frame_ts[0][0])+"_to_"+str(frame_ts[0][1])+".jpg")  # create the save path
...
def video_to_frames(video_path, frames_dir, overwrite=False, every=1, chunk_size=1000,mlType=None, model=None ):
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        futures = [executor.submit(extract_frames, video_path, frames_dir, overwrite, f[0], f[1], every, mlType,model)
...
if __name__ == '__main__':
    ...
    my_parser.add_argument('--ml', metavar='ml', type=str,default='classify',  help='classification or object detection')
    my_parser.add_argument('--model', metavar='model', type=str,default=None, help='ssd, centernet etc..')
    args=my_parser.parse_args()
    print("Args Passed "+ str(args))
    #x=video_to_frames(video_path=args.vid, frames_dir='/home/data/input/frames', overwrite=True, every=5, chunk_size=1000,mlType=args.ml, model=args.model)
    y=extract_frames(video_path=args.vid, frames_dir='/home/data/input/frames', overwrite=True, every=5,mlType=args.ml, model=args.model)
    print(y)
